...
#define CONFIG_MODE_COMBO 3     /* Shouldn't ever happen. */
    int local;                  /* Boolean: "Gami" directive declared
                                 * here?
                                 */
...
{
    return (x_cfg *) ap_get_module_config(r->per_dir_config, &gami_module);
}
...
{
    return (x_cfg *) ap_get_module_config(s->module_config, &gami_module);
}
...
{
    return (x_cfg *) ap_get_module_config(r->request_config, &gami_module);
}
#endif
...
{
    return (x_cfg *) ap_get_module_config(c->conn_config, &gami_module);
}
                      ...
                      const char *note)
{
     ...
     */
#define GAMI_LOG_EACH 0
    if (GAMI_LOG_EACH && (s != NULL)) {
        ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_gami: %s", note);
    }
}
...
/*--------------------------------------------------------------------------*/
/*
 * Command handler for the NO_ARGS "Gami" directive.  All we do is mark the
 * call in the trace log, and flag the applicability of the directive to the
 * current location in that location's configuration record.
 */
static const char *cmd_gami(cmd_parms *cmd, void *mconfig)
{
     ...
     */
    cfg->local = 1;
    trace_add(cmd->server, NULL, cfg, "cmd_gami()");
    return NULL;
}
...
static int x_handler(request_rec *r)
{
    ...
    if (strcmp(r->handler, "gami-handler")) {
        return DECLINED;
    }
    ...
    ap_rputs("<HTML>\n", r);
    ap_rputs(" <HEAD>\n", r);
    ap_rputs("  <TITLE>mod_gami Module Content-Handler Output\n", r);
    ap_rputs("  </TITLE>\n", r);
    ap_rputs(" </HEAD>\n", r);
    ap_rputs(" <BODY>\n", r);
    ap_rputs("  <H1><SAMP>mod_gami</SAMP> Module Content-Handler Output\n", r);
    ap_rputs("  </H1>\n", r);
    ap_rputs("  <P>\n", r);
    ...
    ap_rprintf(r, "   <LI>\"Gami\" directive declared here: %s\n   </LI>\n",
            (dcfg->local ? "YES" : "NO"));
    ap_rprintf(r, "   <LI>\"Gami\" inherited: %s\n   </LI>\n",
...
static const command_rec x_cmds[] =
{
    AP_INIT_NO_ARGS(
        "Gami",                          /* directive name */
        cmd_gami,                        /* config action routine */
        NULL,                               /* argument to include in call */
        OR_OPTIONS,                         /* where available */
        "Gami directive - no arguments"  /* directive description */
    ),
    {NULL}
};
/*--------------------------------------------------------------------------*/
